...
type KeyMap struct {
	...
	Sort      key.Binding
	SortDir   key.Binding
	Kill      key.Binding // F9: send signal
	Pause     key.Binding
	Refresh   key.Binding
	Export    key.Binding
	Help      key.Binding
...
func DefaultKeyMap() KeyMap {
	return KeyMap{
		...
		Kill:      key.NewBinding(key.WithKeys("f9"), key.WithHelp("F9", "signal")),
...
func (k KeyMap) ShortHelp() []key.Binding {
	return []key.Binding{k.Up, k.Down, k.Sort, k.Kill, k.Search, k.Filter, k.Quit}
}
...
func (k KeyMap) FullHelp() [][]key.Binding {
	return [][]key.Binding{
		...
		{k.Search, k.Filter, k.Sort, k.SortDir, k.ToggleSvc, k.Kill},
