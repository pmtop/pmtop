...
// View renders the TUI: top status bar, port table, and bottom hint/status bar.
func (m Model) View() string {
	...
	top := m.style.StatusBar(m.version, m.root, m.paused, intervalString(m.interval), "", width)
	body := m.tbl.View()
...
func (m Model) hints(width int) string {
	...
	for _, b := range m.keys.ShortHelp() {
		...
		// Use only the first key alternative to keep it compact.
